# Scenario Title
DELETE /books/{id} — Response Schema and Contract Validation

# Endpoint Information
- Method: DELETE
- Endpoint: /books/{id}
- Description: `books-controller` operation `deleteById`. Accepts a required path parameter `id` (integer,
  `int64`) and deletes the corresponding book. The only documented response is `204 No Content`, with **no
  `content` object declared at all** for that status — identical in shape to `DELETE /authors/{id}`
  (`deleteById_1`), and distinct from `DELETE /orders/{id}` (`deleteOrder`) in the same spec, whose `204`
  response documents `content: {"*/*": {"schema": {"type": "string"}}}`. This file validates the API
  contract only: status code and the absence of a response body/schema. Business flows (actual deletion
  effect, data consistency, cross-resource impact) and error/negative handling are covered in the positive
  and negative files.
...
- An existing book record must be present (e.g. seeded via `POST /books`, which itself requires at least
  one existing author `id`) so its `id` can be used as the path parameter for a schema-valid call. Because
  `DELETE` removes the resource, a fresh book must be re-seeded for each schema test case that requires a
  valid, still-existing `id` (deletion is destructive, so the resource cannot be reused across test cases).

# Test Data
- Valid path param: `id` of a pre-existing, freshly seeded book (integer, `int64`).
...
## Test Case ID
SCHEMA-BOOKS-DELETE-001

## Scenario
Validate HTTP status code for a successful delete of an existing book.
...
### Path Params
- `id`: a pre-existing, freshly seeded book id.
...
distinct from an endpoint that documents `204` with an explicit (even if generic) content schema (e.g.
`deleteOrder`).
...
### Path Params
- `id`: a pre-existing, freshly seeded book id (integer, `int64`).
...
This test records actual behavior for a second, independent deletion of a different freshly seeded book.

### Headers
None required.

### Path Params
- `id`: a second, independently seeded book id (different from prior test cases).
...
## Expected Response
No response body, consistent with SCHEMA-BOOKS-DELETE-001 through 004.
...
# Notes
- The `204` response for `deleteById` (books) declares **no `content` object at all**, which is a
  stricter, more explicit contract than simply "no schema specified" — it commits to an empty body. This
  matches `DELETE /authors/{id}` (`deleteById_1`) exactly, but is a documented inconsistency versus
  `DELETE /orders/{id}` (`deleteOrder`), which declares `content: {"*/*": {"schema": {"type": "string"}}}`
  on its own `204`. This is flagged as a spec-consistency observation for the API team; it does not change
  the assertions for this endpoint, which follow only what is documented for `deleteById` (books) itself.
- No `requestBody` is declared for this operation, so there are no request-body schema assertions in this
  file (unlike the `PUT`/`PATCH` counterparts for `/books/{id}`).
...
- The only explicit request-side contract constraint is the `id` path parameter's `type: integer, format:
  int64, required: true`, exercised in SCHEMA-BOOKS-DELETE-004 and used as the baseline for negative
  path-parameter type-mismatch cases in the negative scenarios file.
- `Book.authors` is documented as an array of `Author` objects, and `RichOrder.items[].book` documents a
  `Book` reference — i.e. `Book` is a resource pointed to by `OrderItem`, not the other way around, and
  `Book` itself points to (but does not own) `Author`. Neither relationship affects the shape of this
  operation's own `204`/empty-body contract; any referential-integrity effects of deleting a book are
  business-flow concerns and are covered in the positive/negative files, not here.
